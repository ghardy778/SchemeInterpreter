...
namespace Tree
{
        ...
        public Environment(Environment e)
	{
        ...
        private static Node find(Node id, Node alist)
        {
            ...
            else
            {
                ...
                if (id.getName().Equals(bind.getCar().getName()))
                    ...
                    return bind.getCdr(); //returning a cons node with a builtin car
                else
        ...
        public void define(Node id, Node val)
        {
            ...
            Node defNode = new Cons(Nil.getInstance(), Nil.getInstance());
            Node defCadr = new Cons(val, Nil.getInstance()); 
            Node defCar = new Cons(id, defCadr);
            defNode.setCar(defCar);  //Created a cons node of the newly defined variable
            defNode.setCdr(frame);   //Set the existing frame to the cdr
            frame = defNode;         //Set frame to the newly defined Node
        ...
        public void assign(Node id, Node val)
        {
            ...
            Node changeNode = find(id, frame);
            if (changeNode == null && env == null)
            {
                Console.Error.WriteLine("undefined variable " + id.getName());
                
            }
            else if (changeNode == null)
                // look up the identifier in the enclosing scope
                env.lookup(id);
            else
                // get the value out of the list we got from find()
                changeNode.setCar(val);
        }
